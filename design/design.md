...
1. **The comparison thesis.** Trip 1 was instinct travel (no guides existed); trip 2 was curated travel (10-category sourced guides per city). Retroactive guides for trip-1 cities create a control group, and the site's signature question — *does curation change where you end up, or confirm your instincts?* — is answerable with real data ([D2](decision-log.md#d2-retroactive-guides-for-trip-1-portland-excluded)). Trip-1 and trip-2 data are semantically different and are **never visually conflated** ([D3](decision-log.md#d3-distinct-trip-1-status-enum), [D11](decision-log.md#d11-status-color-law)).
...
- `status` enum — trip 2 (guided): `attended` · `planned-skipped` · `off-guide-discovery` · `closed-on-arrival` · `unverified`; trip 1 (retro-guided): `attended-anyway` · `retroactive-recommendation`. Semantics and the never-conflate rule: D3.
...
- This one `status` field powers the waffle charts (attended ÷ planned), the guide hit-rate analysis, off-guide discovery highlights, and the closures ledger (Proof Bakeshop, Hodgepodge, Switched On, Ipanema — the displacement story is content, not just a data-quality footnote).
